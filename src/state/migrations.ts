...
export const backupLegacyState = async (paths: HomePaths, timestamp: string): Promise<string> => {
...
export const removeDatabase = async (database: string): Promise<void> => {
  for (const suffix of ["", "-wal", "-shm", "-journal"]) await rm(`${database}${suffix}`, { force: true });
};

export const pruneDatabaseBackups = async (paths: HomePaths): Promise<void> => {
...
export const requiresStateEpochReset = async (database: string, root: string): Promise<boolean> => {
