...
export const DEFAULT_ENVIRONMENTS: EnvironmentFile[] = [
  { name: "quality", platforms: [...PLATFORMS], defaults: { codex: { model: "gpt-5.5", reasoning: "high" }, claude: { model: "claude-opus-4-6", effort: "high" }, opencode: { model: "openai/gpt-5.4", variant: "high", options: {} } } },
  { name: "balanced", platforms: [...PLATFORMS], defaults: { codex: { model: "gpt-5.2", reasoning: "medium" }, claude: { model: "claude-sonnet-4-6", effort: "medium" }, opencode: { model: "openai/gpt-5.2", variant: "medium", options: {} } } },
  { name: "economy", platforms: [...PLATFORMS], defaults: { codex: { model: "gpt-5.2", reasoning: "low" }, claude: { model: "claude-haiku-4-5", effort: "low" }, opencode: { model: "openai/gpt-5.2", variant: "low", options: {} } } },
];

const MANAGED_START = "<!-- ai-team:managed:start -->";
const MANAGED_END = "<!-- ai-team:managed:end -->";
const FILE_MARKER = "Generated by ai-team. Managed file; local edits block replacement.";
...
const renderBody = (role: Role, platform: Platform, model: ModelConfig, environment: string): string => {
  ...
  const metadata = { role, platform, environment, model, contract_digest: CONTRACT_DIGEST, role_manifest_digest: ROLE_MANIFEST_DIGEST };
  const instructions = [
    `Role: ${role}`,
    `Purpose: ${definition.purpose}`,
    `Allowed commands: ${definition.commands.join(", ")}`,
    `Delegates: ${definition.delegates.join(", ") || "none"}`,
    `Discovery: ${definition.discovery ? "allowed" : "forbidden; request file-explorer support"}`,
    "Claim the dispatch, use the frozen prompt unchanged, validate the result, then submit it.",
    "Stop and return requested_support for any work outside the dispatch packet.",
  ].join("\n");
  if (platform === "codex") {
...
const managedBlock = (environment: string): string => `${MANAGED_START}\nUse ai-team managed planning or coding agents for explicit AI Team workflows. Active environment: ${environment}.\n${MANAGED_END}`;
