...
export const COMMAND_SYNTAX: Readonly<Record<string, readonly string[]>> = Object.freeze({
  ...
  "dispatch create": ["ai-team dispatch create --run-id <run-id> --role <role> --actor-role <role> [--actor-dispatch-id <dispatch-id>] (--packet-file <json> | --staging-id <staging-id>)"],
  ...
  "dispatch supersede": ["ai-team dispatch supersede --run-id <run-id> --dispatch-id <dispatch-id> --role <role> --actor-role <role> --reason <text> (--packet-file <json> | --staging-id <staging-id>)"],
  ...
  "decision create": ["ai-team decision create --run-id <run-id> --dispatch-id <dispatch-id> (--file <json> | --staging-id <staging-id>)"],
  ...
  "git reconcile": ["ai-team git reconcile --run-id <run-id> --dispatch-id <dispatch-id> [--operation-id <opaque-id> --state <state> (--evidence-file <json> | --staging-id <staging-id>)]"],
  "git cleanup": ["ai-team git cleanup --run-id <run-id> --dispatch-id <dispatch-id>"],
  "research archive": ["ai-team research archive --run-id <run-id> --project <path> --topic <text> (--report-file <json> | --staging-id <staging-id>)"],
...
const AGENT_COMMAND_SYNTAX_OVERRIDES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  ...
  "dispatch supersede": ["ai-team dispatch supersede --run-id <run-id> --dispatch-id <dispatch-id> --role <role> --actor-role <role> --reason <text> --staging-id <staging-id>"],
...
const AGENT_COMMANDS = ["context update", "context validate", "planning revision validate", "planning revision create", "planning revision transition", "planning revision commit", "planning tasks validate", "dispatch create", "dispatch claim", "dispatch cancel", "dispatch reissue", "dispatch supersede", "dispatch prompt", "dispatch schema", "dispatch template", "dispatch validate", "dispatch submit", "decision create", "decision schema", "decision template", "staging create", "staging write", "staging show", "staging cleanup", "scope check", "git status", "git prepare", "git adopt", "git transfer", "git commit", "git merge-task", "git integrate", "git reconcile", "git cleanup", "research archive", "review create", "review submit", "review resolve", "review status"] as const;
...
export const COMMAND_VALIDATORS: Readonly<Record<string, CommandSpec>> = Object.freeze({
  "context.update": { required: ["project"], optional: ["contextFile", "stagingId", "runId"], exclusive: [["contextFile", "stagingId"]], patterns: { stagingId: IDS.stagingId, runId: IDS.runId }, ...((COMMAND_SYNTAX["context update"]) ? { syntax: COMMAND_SYNTAX["context update"] } : {}) },
