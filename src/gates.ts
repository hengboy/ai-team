...
import { ensureGitPrepareDispatch } from "./dispatch/task-lifecycle.js";
import { assertPreCommitScope, checkScope, plannedWorktreeSnapshot } from "./dispatch/store.js";
...
export { plannedWorktreeSnapshot };
...
export class ScopeGate {
  ...
  check(runId: string, stage: ScopeGateStage, paths: string[], worktreeId?: string, diagnostics: Record<string, unknown> = {}): { digest: string; complete: boolean } {
    ...
    if (!direct) {
      ...
      return checkScope(this.store, runId, stage, paths, worktreeId, diagnostics);
    }
    ...
    if (existing.some((item) => item.stage === stage)) {
      if (stage === "pre_write") ensureGitPrepareDispatch(this.store, dispatchOperations, runId, "implementation");
  ...
  assertPreCommit(runId: string, paths: string[], worktreeId: string): void {
    assertPreCommitScope(this.store, runId, paths, worktreeId);
  }
