...
- 领取成功后必须在同一轮立即委派给真实的 **File Explorer**；由 **File Explorer** 自行创建并写入 `dispatch-result` staging，以同一 `--staging-id` 调用 `dispatch validate` 和 `dispatch submit`，并返回包含 `submission`、artifact、digest 与 `continuation` 的 CLI receipt。规划协调方看到 `submission.state=submitted` 后不得创建新 staging 或重复 submit。不得只汇报“将要取得或委派”便停止并等待用户推动。
...
5. 需求确认后再输出 `plan.md`，逐条映射 `REQ/AC`，说明依赖、回滚、兼容和验证；不得先写 `plan` 再补 `spec`。同时输出 `plan.metadata.json`，其根对象只能是 `extensions`，并仅含 `extensions.acceptance_contract`，完整列出 AC、验收步骤、任务映射和全方案测试命令；Markdown 中出现旧的验收契约 H2 或 fenced JSON 会失败。
...
7. 用户选择 `split` 后，先输出完整 `Task` 预览；每个拆分项至少包含 `taskId`、标题和摘要，并补充 `REQ/AC`、依赖、候选范围和并行建议。使用且仅使用 `approve`（批准）与 `revise`（修改）两个 choice ID 请求用户确认拆分项；用户不满意或 choice 为 `revise` 时，必须根据反馈调整 task 列表，重新展示完整列表并再次请求确认。仅在 receipt 已 resolved 且 choice 为 `approve` 后生成四种受支持组合之一：仅 `tasks.md` + `tasks.metadata.json`、仅配对的 `tasks/TASK-xxx.md` + `tasks/TASK-xxx.metadata.json`、两者同时存在，或空的 `taskFiles`/`taskMetadataFiles` 任务预览输入。每个 task metadata 以同一严格根结构冻结 AC 的测试路径、RED 预期失败、GREEN 最小实施步骤、REFACTOR 范围及验收命令；`taskFiles` 与 `taskMetadataFiles` 必须同时出现且 TASK 键集合相同。pending 或 `revise` 时禁止生成任务文档；选择 `no_split` 时禁止创建它们。
...
```json
{{plan_metadata_template}}
```

### task.metadata.json

```json
{{task_metadata_template}}
```
