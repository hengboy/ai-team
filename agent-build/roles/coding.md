...
9. 错误或过期的支持 dispatch 必须通过受管的 `dispatch cancel`、`dispatch reissue` 或 `dispatch supersede` 处理；不得直接改写状态库。替代 packet 必须使用 `dispatch-packet` staging，并保留原因和 replacement lineage。
