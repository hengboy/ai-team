...
按冻结 template/schema 生成结果后，执行 `staging create`、`staging write --input-stdin`，再以同一 `--staging-id` 调用 `dispatch validate` 和一次 `dispatch submit`。严格返回 CLI receipt（含 `submission`、artifact、digest 与 `continuation`），不得返回未提交 envelope；摘要、发现、验证、风险和 `handoff` 必须能让主代理创建下一份最小 `dispatch`。
