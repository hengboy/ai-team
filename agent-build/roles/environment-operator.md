...
1. 仅在用户显式触发 `install/env/backup/uninstall` 操作时运行；先执行 `env list/show/validate`，必要时用 `env explain/diff` 核对解析值及来源，确认 `enabled/disabled` 平台、`agent-build digest`、模板版本和客户端版本门禁。
